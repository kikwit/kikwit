...
export const requestMaxSizeProp = Symbol();
...
function configureRoutes(ctrlFile, ctrl) {
    ...
    for (let action of ctrl[actionsProp]) {
        ...
        route = {
            ...
            requestMaxSize: actionDecorators.requestMaxSize || ctrlDecorators.requestMaxSize,
            services: {}
        };
...
function getDecorators(target) {
    ...
    return {
        ...
        requestMaxSize: target[requestMaxSizeProp],
...
function getRoutePath(controllerBasename, controllerRoute, actionName, actionRoute) {
    ...
    routePath = routePath.replace(/\[controller\]/g, controllerBasename);

    if (actionName) {
        routePath = routePath.replace(/\[action\]/g, actionName);
    }
...
function configureStaticFiles() {
    ...
    if (appConfig.staticFiles) {
        ...
        if (appConfig.staticFiles.virtualPath
            && appConfig.staticFiles.virtualPath.trim()) {
             setStaticFilesVirtualPathRegExp();
        }

        if (appConfig.staticFiles.headers
            && typeof(appConfig.staticFiles.headers) == 'function') {

            const oldFn = appConfig.staticFiles.headers;

            appConfig.staticFiles.headers = (file) => {

                const start = 2
                    + appConfig.applicationRoot.length
                    + appConfig.staticFiles.root.length;

                const relativePath = file.slice(start);

                return oldFn(relativePath);
            }
        }
    }
}

export function setStaticFilesVirtualPathRegExp() {

    let virtualPath = appConfig.staticFiles.virtualPath;
    ...
    if (virtualPath.endsWith('/')) {
        virtualPath = virtualPath.slice(0, virtualPath.length - 1);
    }

    let virtualPathRegExp;

    switch (os.platform()) {

        case 'win32':
            virtualPathRegExp = new RegExp(`^${virtualPath.replace('/', '\\\\')}(?:\\\\(.*))?`, 'i');
            break;

        default:
            virtualPathRegExp = new RegExp(`^${virtualPath}(?:/(.*))?`, 'i');
            break;
    }

    appConfig.staticFiles.virtualPathRegExp = virtualPathRegExp;
}
