...
export default {
	...
	render(viewPath, locals, contentType) {
		...
		const actionName = this.route.actionName;
		const controllerName = this.route.Controller.name;
		const response = this.response;

		const viewsSettings = this.appSettings.views;
		...
		this.result = new results.ViewResult(ctx, viewPath, locals, contentType);
		this.resolve();
	},
