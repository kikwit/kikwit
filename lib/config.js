...
export function init(settings) {
	...
	appSettings = mergeRecursive(appSettings, settings);

	appSettings.environment = settings.environment || process.env.NODE_ENV || defaultSettings.environment;

    if (appSettings.staticFiles && appSettings.staticFiles.root) {
        setStaticFilesRootRegExp();
    }

	const controllersRoot = appSettings.controllersRoot || defaultSettings.controllersRoot;
    const dirPath = path.join(process.cwd(), controllersRoot);
