...
export function setRoute(ctx) {
    ...
    const result = router.findRoute(ctx);
    ...
    ctx.matchedVersion = result.route.version;
...
export function injectServices(ctx, services) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const injectErrFn = (key) => { throw new Error(`Invalid dependency [${key}] for route [${route.routePath}]`) };
