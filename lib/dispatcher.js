...
import WebSocketResult from './actionResults/webSockets/webSocketResult';
...
export function dispatchWebSocket(config, trustProxy, routes, services, webSocket) {
    ...
    injectServices(ctx, services)
        ...
        .then(() => {
            ...
            runBeforeInterceptors(ctx)
                .then(executeResult)
                .catch(err => {
                    if (err) {
                        ctx.resolve = Promise.resolve;
                        ctx.close(err.code || 1011, err.message);
                    }
                });             
...
function executeResult(ctx) {
    ...
    return result.run()
        .then(() => {
            ...
            } else if (result instanceof WebSocketResult
                && !ctx.connectionClosed) {
