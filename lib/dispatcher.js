...
export function setRequestProperties(ctx) {
    
    const request = ctx.request;
    let host, hostname, port, protocol, ips, ip;
    ...
    if (ctx.appSettings.trustProxy) {
        ...
        console.dir({protocol, ips, port});      
    }
        
    if (!protocol || !protocol.trim().length) {
    ...
    if(port && port.trim().length) {
        ...
        port = ctx.appSettings.port;
    }
    
    host = `${hostname}:${port}`;
    
    Object.assign(ctx.request, { host, hostname, port, protocol, ips, ip });
}
